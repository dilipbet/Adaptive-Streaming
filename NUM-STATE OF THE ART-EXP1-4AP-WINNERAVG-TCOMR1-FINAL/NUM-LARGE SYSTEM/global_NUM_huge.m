...
[cap_mat,cap_bs,active_links,zz_bs] = physical_layer(z_h,z_u,macro_side,cell_side,helpers_per_cell,users_per_cell...
    ,W_lte,W_wifi,freq_reuse,slot_duration,no_helper,num_helpers,num_users);
%load('peakrates5.mat','cap_mat');
assoc = (cap_mat == repmat(max(cap_mat,[],1),num_helpers,1));
num_best1 = sum(assoc,1);
num_best1_rep = repmat(num_best1,1,num_helpers);
vv = repmat((num_best1>1),num_helpers,1);
temppp = cumsum(assoc.*vv);
index = (temppp==1);
assoc(vv) = index(vv);
num_best2 = sum(assoc,1);
num_best2_rep = repmat(num_best2,1,num_helpers);
vv2 = repmat((num_best2>1),num_helpers,1);
temppp2 = cumsum(assoc.*vv2);
index2 = (temppp2==1);
assoc(vv2) = index2(vv2);
