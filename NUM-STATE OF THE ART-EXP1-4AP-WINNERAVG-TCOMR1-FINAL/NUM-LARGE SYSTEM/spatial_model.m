...
%****UNIF DISTRIBUTION IN EACH SMALL CELL**********
...
%***********NON-UNIFORM DISTRIBUTION ACROSS SMALL CELLS*******
x_u = zeros(1,num_users);
x_u(1:num_users) = unifrnd(-5,-3,1,num_users);
%x_u(1:num_users-5) = unifrnd(-5,-2.5,1,num_users-5);
%x_u(num_users-4:num_users) = unifrnd(-5,0,5);

%x_u = x_u + 20;
%*****y-coordinates of users*****
%******UNIFORM Y-COORIDNATE*********
...
%*****NON-UNIFORM Y-COORDINATE*********
 y_u = zeros(1,num_users);
 y_u(1:num_users) = unifrnd(-5,-3,1,num_users);
% y_u(1:num_users-5) = unifrnd(-5,-2.5,1,num_users-5);
% y_u(num_users-4:num_users) = unifrnd(-5,0,5);
%y_u = y_u + 20;
z_u = complex(x_u,y_u);
