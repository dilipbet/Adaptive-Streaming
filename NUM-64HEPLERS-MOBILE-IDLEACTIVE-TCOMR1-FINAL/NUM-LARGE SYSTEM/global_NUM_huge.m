...
macro_side = 40;                           % side length of macro square grid
cell_side = 5;                             % side length of cell square
...
N = 3000;                                    %number of scheduling slots for which we 
...
remainder_list = zeros(N,num_users,length(param));
chunk_deadlines_list = zeros(N,num_users,length(param));
bs_queues = zeros(N+1,num_users,length(param));
timeaveutil = zeros(1,length(param));
timeavetotbacklog = zeros(1,length(param));
dline_delay_list = zeros(N,num_users,length(param));
num_interr_list =  zeros(length(param),num_users);
frac_num_interr_list = zeros(length(param),num_users);
active_users_list = zeros(N,num_users,length(param));
time_active_list  = zeros(length(param),num_users);
...
%rand_start(1:4) = [1 201 401 601]; %since we have 4 videos concatenated one after the other
                                   ...
                                   %resply
chunk_duration = 0.5;              % each GOP/chunk has duration 2/3 seconds;
...
%[z_h, z_u,num_helpers,num_users,r] = BS_user_placement;
%save('topologyreview.mat','z_h','z_u');
%% *****SETTING UP PHYSICAL LAYER*****
[cap_mat,cap_bs,zz,active_links,zz_bs,total_symbols] = physical_layer(z_h,z_u,macro_side,cell_side,helpers_per_cell,users_per_cell...
    ,W_lte,W_wifi,freq_reuse,slot_duration,no_helper);
tt = zz;
tt_bs = zz_bs;
%tt_bs = zeros(1,length(zz_bs));
%% 
...
chunk_number_requested = zeros(1,num_users);
 ...
 active_users=binornd(1,prob_req,[1,num_users]);
 active_users(1) = 1;
 %active_users(even_vec)=0;
 %zz = tt.*repmat(active_users,num_helpers,1);
 %zz_bs = tt_bs.*active_users;
 request_list = zeros(N,num_users);
 path_x = zeros(1,N);
 path_y = zeros(1,N);
 allones = ones(num_helpers,num_users);
 active_users_mat = zeros(N,num_users);

 %% ********BEGINNING THE DYNAMIC SIMULATION FOR N TIME SLOTS...
 for i = 1:N
     active_users_mat(i,:) = active_users;
...
%     end
    ...
    %,W_lte,W_wifi,freq_reuse,slot_duration,no_helper);
     ...
     mobile_dist = helper_user_distance(:,1);
     mobile_pathloss = winner(mobile_dist);
     mob_path_loss = 10.^(-mobile_pathloss/10);
    

 helper_power = repmat(10^8,num_helpers,1);
 helper_user_strength = (helper_power).*mob_path_loss;
 ...
 mobile_SINR = desired_signal_power./(interf_power+1);
 cap_mobile = total_symbols*exp(1./mobile_SINR).*expint(1./mobile_SINR);
 cap_mobile(isnan(cap_mobile)) = 0;
 cap_mobile(isinf(cap_mobile)) = 0;
 cap_mat(:,1) = cap_mobile;
 zz = (cap_mat>10^6);
     end
    ...
    %*****gamma decisions******
    [gamma] = gammadec(theta,zz_bs,V,active_users);
   ...
   %% *************DEMANDS WITH A CERTAIN REQUEST PROBABILITY*********
    [active_users] = demand(i,active_users,chunk_number_requested);
    %req_time_stamp_list(i,:) = req_time_stamp;
    %zz = tt.*repmat(active_users,num_helpers,1);
    %zz_bs = tt_bs.*active_users;
end
%%
...
[slot_index,user_index,allchunk_reception_profile,helper_assign,rrr,served_bits]...
     = playback_buffers(rr,mumu,num_users,N,num_helpers,trans,active_users_mat);
alluser_chunk_delay = allchunk_reception_profile - request_list;
...
for i = 1:num_users
    ...
    if(length(buff_start_list) - length(play_start_list) > 0)
        play_start_list = [play_start_list;3000];
    end
    prefetch_ind = false(length(buff_start_list),1);
    for jjj =1 : length(prefetch_start)
       prefetch_ind = prefetch_ind | (buff_start_list == prefetch_start(jjj));
    end
    prefetch_time = play_start_list(prefetch_ind) - buff_start_list(prefetch_ind);
    buffering_time = play_start_list(~prefetch_ind) - buff_start_list(~prefetch_ind);
    if(i==1)
        bibi = startup_time_sess(1:num_sessions(1));
        bibi(1:length(prefetch_time)) = prefetch_time';
        startup_time_sess(1:num_sessions(1)) = bibi;
    else
        bibi = startup_time_sess(cum_num_sessions(i-1)+1:cum_num_sessions(i));
        bibi(1:length(prefetch_time)) = prefetch_time';
        startup_time_sess(cum_num_sessions(i-1)+1:cum_num_sessions(i)) = bibi;
    end
    not_prefetch_ind = ~prefetch_ind;
    
    
    total_rebuff_time(i) = sum(buffering_time);
    num_rebuff(i) = length(buffering_time);
    ave_rebuff_time(i) = mean(buffering_time);
    startup_time(i) = mean(prefetch_time); 
end
  total_play_time = sum(active_users_mat) + sum((mod(chunk_number_list,1000)==0).*(pb_buffers>0));
...
reception_profile(:,:,jj) = allchunk_reception_profile;
%remainder_list(:,:,jj) = remainder;
%chunk_deadlines_list(:,:,jj) = chunk_deadlines;
helper_assign_eachv(:,:,jj) = helper_assign;
bs_queues(:,:,jj) = bs_evolve;
timeaveutil(jj) = sum(mean(d_evolve,3));
%active_users_list(jj,:) = active_users;
%sum(log(1+mean(d_evolve,3)));
...
%hold all
%figure(2)
%ecdf(num_chunks_requested);
%hold all
figure(1)
plottoplarge
scatter(path_x, path_y, 'og')

figure(2)
cdfplot(frac_dropped*100)
xlabel('percentage of skipped chunks (x)')
ylabel('fraction of users with % of skipped chunks < x ')
hold all

figure(3)
cdfplot(startup_time)
xlabel('pre-buffering time (x)')
ylabel('fraction of users with pre-buffering time < x')
hold all

%figure(3)
%ecdf(total_rebuff_time)
%hold all

%figure(6)
%ecdf(ave_rebuff_time)
%hold all

figure(4)
cdfplot(num_rebuff./num_sessions)
...
figure(7)
plot(pb_buffers(1:1000,1))
xlabel('time slot')
ylabel('playback buffer size')
hold all

figure(8)
stem(helper_assign(:,1))
xlabel('chunk number')
ylabel('helper assigned')


% active_users_list(:,:,jj) = active_users_mat;
% time_active = sum(active_users_mat,1);
% time_active_list(jj,:) = time_active;
% figure(3)
% ecdf((sum(d_evolve,3))./time_active);
% %ecdf(mean(d_evolve,3));
% hold all
% lost_chunk_ind = (alluser_chunk_delay(1:N-50,:)==inf);
% lost_chunk_indices = cumsum(lost_chunk_ind);
% transient_time = sum(lost_chunk_indices < repmat(max(lost_chunk_indices),N-50,1));
% trans_time_list(jj,:) = transient_time;
% figure(4)
% ecdf(transient_time)
% hold all% steady_state_id = (cumsum(ones(N,num_users)) > repmat(transient_time,N,1));
% worst_delay = max(alluser_chunk_delay.*(alluser_chunk_delay~=inf).*steady_state_id);
% worst_delay_list(jj,:) = worst_delay;
% figure(5)
% ecdf(worst_delay)
%making_idle_zero1 = chunk_deadlines;
%making_idle_zero1(helper_assign==0) = 0;
%making_idle_zero2 = alluser_chunk_delay+remainder;
%making_idle_zero2(helper_assign == 0) = 0;
%deadline_delay_diff = making_idle_zero1-making_idle_zero2;
%interrupt = (deadline_delay_diff<0);

%num_interrupt = sum(interrupt,1);
%frac_num_interrupt = num_interrupt./time_active;
%dline_delay_list(:,:,jj) = deadline_delay_diff;
%num_interr_list(jj,:) = num_interrupt;
%frac_num_interr_list(jj,:) = frac_num_interrupt;
%% *************************************************************
%****************************************************
 
%  play_start = cumsum(ones(N,num_users),1) > repmat(start_point,N,1);
...
jj = jj+1;
popo=1;
[chunk_number_list(:,popo) request_list(:,popo) helper_assign(:,popo) allchunk_reception_profile(:,popo) alluser_chunk_delay(:,popo)...
  first_avail(:,popo) new_chunks_avail(:,popo) last_playable_chunk(:,popo) newly_avail_for_play(:,popo) drop_list(:,popo) max_delay_window(:,popo) pb_buffers(:,popo) buff_start_time(:,popo) play_start_time(:,popo) ];  
