...
macro_side = 10;                           % side length of macro square grid
...
%adding extra fake layer of 0 bits and 0 quality value
v111 = zeros(total_gop*9,4);
v222 = zeros(total_gop*9,4);
v333 = zeros(total_gop*9,4);
v444 = zeros(total_gop*9,4);
for tt = 1: total_gop
    v111((tt-1)*9+1,:)=[tt 0 0 0];
    v111((tt-1)*9+2:(tt-1)*9+9,:) = v1((tt-1)*8+1:(tt-1)*8+8,:);
    v222((tt-1)*9+1,:)=[tt 0 0 0];
    v222((tt-1)*9+2:(tt-1)*9+9,:) = v22((tt-1)*8+1:(tt-1)*8+8,:);
    v333((tt-1)*9+1,:)=[tt 0 0 0];
    v333((tt-1)*9+2:(tt-1)*9+9,:) = v33((tt-1)*8+1:(tt-1)*8+8,:);
    v444((tt-1)*9+1,:)=[tt 0 0 0];
    v444((tt-1)*9+2:(tt-1)*9+9,:) = v4((tt-1)*8+1:(tt-1)*8+8,:); 
end
...
xx = [v111;v222;v333;v444];% we concatenate the tables of all video trace files to a single table
...
frac_dropped_ensemble = zeros(num_realizations,num_users);
startup_time_ensemble = zeros(num_realizations,num_users);
num_rebuff_ensemble = zeros(num_realizations,num_users);
frac_rebuff_time_ensemble = zeros(num_realizations,num_users);
timeave_ssim_ensemble = zeros(num_realizations,num_users);


num_chunks_received_ensemble = zeros(num_realizations,num_users);


avg_delay_ensemble = zeros(num_realizations,num_users);


max_delay_ensemble = zeros(num_realizations,num_users);


for ensemble = 1:num_realizations                                       
...
%***LATEST***
 ...
 %save('topologytheatre5.mat','z_h','z_u');
...
%*****LATEST*******
    [cap_mat,cap_bs,zz,active_links,zz_bs,helper_user_distance,helper_user_strength,...
        desired_signal_power,interf_power,pathloss_SINR,path_loss,path_loss1,pathloss,total_symbols] = physical_layer(z_h,z_u,macro_side,...
        cell_side,helpers_per_cell,users_per_cell...
        ,W_lte,W_wifi,freq_reuse,slot_duration,no_helper,num_helpers,num_users,r);
    
[r c] = find(zz);
edge_indices = [r c];
A = zeros(num_helpers+num_users,length(r));
b = ones(num_helpers+num_users,1);
for indind = 1 : length(r)
    A(edge_indices(indind,1),indind) = 1;
    A(num_helpers+edge_indices(indind,2),indind) = 1;
end

   %save('peakrates5','cap_mat');

%****LATEST ENDS*******
%load('peakrates5','cap_mat');
%zz = (cap_mat>10^6);
 tt = zz;
% zz_bs = true(1,num_users);
 %cap_bs = zeros(1,num_users);
...
%     ,W_lte,W_wifi,freq_reuse,slot_duration,no_helper);
    ...
    [mu,mu_bs,exitflag] = scheduler(z,cap_mat,zz,bs_queue,cap_bs,no_helper,zz_bs,num_helpers,num_users,A,b,edge_indices);
    exitlist(i) = exitflag;
...
% %delay_profile(delay_profile==inf) = 0;
 ...
 for i= 2:N
     ...
     max_delay_window(max_delay_window==0)=inf;
     temp3 = max_delay_window(i,:);
     new_startup = ((pb_buffers(i-1,:) == 0) & (start==1)& (mod(chunk_number_list(i,:),N)~=0));
...
num_sessions = ceil(num_chunks_requested./N);
...
fracdrop = zeros(N/N,num_users);
for hh = 1:N/N
 fracdrop(hh,:) = sum(ceil(dropped_chunks/N)==hh)/N;   
end
for ppp = 1:num_users
    if(ppp ==1) 
...
frac_dropped_ensemble(ensemble,:) = frac_dropped;
startup_time_ensemble(ensemble,:) = startup_time;
num_rebuff_ensemble(ensemble,:) = num_rebuff;
frac_rebuff_time_ensemble(ensemble,:) = frac_rebuff_time;
timeave_ssim_ensemble(ensemble,:) = timeave_ssim;

num_chunks_received = sum(allchunk_reception_profile~=inf);
num_chunks_received_ensemble(ensemble,:) = num_chunks_received;

delay22 = alluser_chunk_delay;
delay22(delay22==inf) = 0;
avg_delay = sum(delay22)./num_chunks_received;
avg_delay_ensemble(ensemble,:) = avg_delay;

max_delay = max(delay22);
max_delay_ensemble(ensemble,:) = max_delay;

ensemble
end
figure(2)
cdfplot(mean(frac_dropped_ensemble,1)*100)
xlabel('percentage of skipped chunks (x)')
ylabel('fraction of users with % of skipped chunks < x ')
hold all

figure(3)
cdfplot(mean(startup_time_ensemble,1))
xlabel('pre-buffering time (x)')
ylabel('fraction of users with pre-buffering time < x')
hold all

figure(4)
cdfplot(mean(num_rebuff_ensemble,1))
xlabel('number of re-buffering periods (x)')
ylabel('fraction of users with no. of re-buffering periods < x')
hold all

figure(5)
cdfplot(mean(frac_rebuff_time_ensemble,1)*100)
xlabel('percentage of playback time spent in re-buffering mode (x)')
ylabel('fraction of users with % of re-buffering < x')
hold all

figure(6)
cdfplot(mean(timeave_ssim_ensemble,1))
...
hold all
figure(7)
cdfplot(mean(num_chunks_received_ensemble,1))


hold all
figure(8)
cdfplot(mean(avg_delay_ensemble,1))


hold all
figure(9)
cdfplot(mean(max_delay_ensemble,1))
