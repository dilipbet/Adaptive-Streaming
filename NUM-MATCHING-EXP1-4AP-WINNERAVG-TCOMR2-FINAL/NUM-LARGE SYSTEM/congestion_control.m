...
%% ******* FOR VBR**********************************
dvec_t = zeros(1,9);
quality = cumsum(ones(9,1),1);
qual_rep = repmat(quality,1,x(2));
rand_start_rep = repmat(rand_start,9,1);
rand_start_rep = mod((rand_start_rep+i-1),total_gop*4);
rand_start_rep(rand_start_rep==0) = total_gop*4;
oppa = cumsum(repmat(ones(9,1),1,num_users),1);
lin_indices1 = 2*total_gop*4*9 + 9*(rand_start_rep-1)+oppa;
lin_indices2 = 3*total_gop*4*9 + 9*(rand_start_rep-1)+oppa;
